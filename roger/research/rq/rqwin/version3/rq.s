...
function(x, y, tau = -1, alpha = 0.10000000000000001, dual = F, int = T, tol
  =.Machine$double.eps^(2/3), ci = T, method="score", interpolate = T, tcrit = T, hs=T)
{
#function to compute regression quantiles
#if(!is.loaded(symbol.For("rqd"))) dyn.load("c:\\junk\\rqd.obj")
#dll.load("rq.dll",symbols="rq")
 if((tau<=0 || tau>=1) && ci){
 ...
 if(missing(x)) {
  ...
  if(int) 
   ...
   warning("method has been set to ``sparsity'' to compute confidence interval when x has only one column 
with no intercept")
   method <- "sparsity"
   }
  }
 ...
 if(tau < 0 || tau > 1) {
  ...
  dimnames(z$sol) <- list(c("Probability", "Quantile", xn), NULL)
  ...
  if(ci){
   if(method=="score") {
    ...
    z$dsol <-matrix(z$dsol, n, z$lsol)
    z$h <- matrix(z$h, p, z$lsol)
    se <-diag(rq.omega(z, tau, hs=hs)*xxinv)^0.5
    Tci<-matrix(0,2,p)
    dimnames(Tci)<-list(c("Lower Bound",
    "Upper Bound"),NULL)
...
function(v, score = "wilcoxon")
{
 ...
 else if(is.numeric(score) && 0 < score && score < 1) {
  isol <- sum (v$sol[1, ] < score)
  w <- (score- v$sol[1, isol])/(v$sol[1, isol + 1] - v$sol[1, isol])
  r <- w * v$dsol[, isol + 1] + (1 - w) * v$dsol[, isol]
  r <- r - (1 - score)
  return(ranks = r,A2 = score * (1 - score))
 }
...
function(x0, x1, y, v, score = "wilcoxon")
{
 if(missing(v) || is.null(v$dsol))
  v <- rq(x0, y, dual = T, ci = F, int = F)
 r <- ranks(v, score)
 x1hat <- as.matrix(qr.resid(qr(x0), x1))
 sn <- as.matrix(t(x1hat) %*% r$ranks)
 Tn <- t(sn) %*% solve(crossprod(x1hat)) %*% sn/r$A2
 return(Tn, sn, rank = r$ranks)
}
