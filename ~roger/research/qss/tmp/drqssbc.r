subroutine drqssbc(nvec,nact,ifl,mxs,psw,e,ner,x,f,erql1n,res,indx,w,nt,nsol,sol,tl,toler,big,eps,penpar1,icyc,tmin,k,k0)
#
# Version 8/28/92
#front end program for dcrql1lt.r 
#
integer ifl,indx(1),mxs,nact,ner,nt
integer nvec(1),nsol,icyc,k,k0
logical psw
double precision e(ner,1),erql1n(1),f(1),res(1),w(1),x(1)
double precision sol(nvec(5)+6,nsol),tl(1),toler,big,eps,penpar1
double precision tmin
#
#     ////////////////  begin program  /////////////////////////
#
call dcrql1lt(nvec(1),nvec(2),nvec(3),nvec(4),nvec(5),nact,ifl,mxs,psw,e,ner,x,f,
erql1n,res,indx,w,nt,nsol,sol,tl(1),tl(2),toler,big,eps,penpar1,icyc,tmin,k,k0)
return
end



subroutine dcrql1lt(nrq,nl1,neqc,niqc,nvars,nact,ifl,mxs,psw,e,ner,x,f,erql1n,res,
indx,w,nt,nsol,sol,t,lam,toler,big,eps,penpar1,icyc,tmin,k,k0)
#
integer ifl,indx(1),mxs,nact,neqc,nrq,nl1,ner,niqc,nvars,nrql1,nt
integer nsol,k,k0
...
double precision tnxt,lnxt
#
#
integer ddx,grdx,grd1x,icyc,iaddc,idelc
integer px,ptex,rrx,topx,zzx
double precision alpha,amag,cgmag,pen,penpar,penpar1,told
#
data zero/0.d00/
data one/1.d00/
#
...
#
#
#penpar=penpar1 #Note: penpar is assigned outside the loop as well as drql1sup
penpar=one/lnxt**.5
repeat {
...
#
		call drql1nlt(nt,tmin,tmax,res,e,f,ner,indx,nact,nvars,
			nrq,nl1,neqc,niqc,w(zzx),nvars,w(rrx),w(grdx),w(px),
			w(topx),w(topx+nvars),ifl,idelc,iaddc,icyc,alpha,amag,
			cgmag,psw,penpar,nsol,sol,x,ilfix,l0,l1,tnxt,lnxt,toler,
			erql1n,eps,big,told,k0)
#		penpar=penpar1  #reinitialize penpar
		penpar=one/lnxt**.5 #update penpar to the sqrt of next lambda
		if (ifl != 0)
			break 1
		}
...
neqc,niqc,zz,nzzr,rr,a,aa,b,bb,ifl,idelc,iaddc,icyc,alpha,amag,cgmag,psw,penpar,
nsol,sol,x,ilfix,l0,l1,tnxt,lnxt,toler,erql1n,eps,big,told,k0)
#
logical fail,psw,ilfix
integer nt,nact,nvars,nrq,nl1,ner,nzzr,ifl,k,nactp1
integer nrql1,neqc,niqc,nallq,nalqp1,ncols,nqnp1,ix
...
if (ilfix){
	if ((ifl==1|ifl==3) && tnxt<tmax){
		...
		sol(5,nt-1) = penal
		sol(6,nt-1) = k
#		tnxt = tnxt+(toler+toler/dabs(two-bb(isave)))
		if (indx(isave) <= nrql1)
			tnxt = tnxt+(toler)*amag
		else
			tnxt = tnxt+(toler)*cgmag
		told = tnxt
...
#		nt = nt - 1
	...
	if ((ifl==1|ifl==3) && lnxt>l0){
		...
		sol(6,nt-1) = k
		if (indx(isave) <= nrql1)
			lnxt = lnxt-(toler)*amag
		else
			lnxt = lnxt-(toler)*cgmag
		call scopy1(nvars,x,1,sol(7,nt-1),1)
		idelc = 0
		iaddc = nact
		icyc = -1
...
#
	...
	if (1<=nrql1) {
		do j = 1,nrql1 {
			...
			do i = 1,nvars
				tmp = tmp+dabs(e(i,j))
			if (tmp<=zero)  #? changed to the following
				{call intpr("j1",2,j,1)
				go to 10
				}
			if (tmp>amag)
...
subroutine dmonit(nact,neqc,niqc,nvars,icyc,psw,x,alpha,erql1n,pen,penpar,indx)
#
integer icyc,indx(1),nact,neqc,niqc,nvars
logical psw
double precision alpha,erql1n,pen,penpar,x(1)
#
#     ***************
#     cl1  version.
#
#     print out information at
#     each step if  psw = .true.
#
#     to change the output medium,
...
#
if (psw) {
	write(nxoutp,50)icyc,alpha
	write(nxoutp,60)(x(i),i = 1,nvars)
	write(nxoutp,70)nact
	if (nact>0) {
		write(nxoutp,80)
		write(nxoutp,90)(indx(i),i = 1,nact)
		}
	write(nxoutp,100)erql1n
	if (neqc+niqc>0)
		write(nxoutp,110)penpar,pen
	}
return
#
50  format("0***** cycle number ",i5/6x,"step taken = ",1pe15 .7/6x,"x-vector...")
60  format(5x,3(1pe15 .7))
70  format(6x,"number of active equations/constraints",i5)
80  format(6x,"active equations/constraints...")
90  format(5x,7i5)
100  format(6x,"residual l1 norm = ",1pe15 .7)
110  format(6x,"penalty parameter = ",1pe10 .2/6x,"penalty function value = ",1pe15 .7)
end
...
#
			...
			do i = 1,num {
				...
				if (ix<=nrql1)
					tmp = tmp*penpar
#				if (ix<=nrq)
#					tmp = tmp - two*(one - two*theta)*penpar
...
#		res(i) = f(ix)
#		}
#	call dzdrgit(nvars,nact,zz,nzzr,rr,res,x,fail,w,big,eps)
#DEBUG}
	if (fail){
		ifl = 4
		}
...
#		else if (tmp>ope+eps-tmpsav && ix>nallq){
#			idelc = irand
#			s = dsign(one,cf)
#			tmpsav = ope+eps-tmp
#			}
