...
depicts the cube root of the mass associated with support points around 8. The smooth red 
curve superimposed in the left panel corresponds to the empirical parametric Bayesian 
estimates assuming $G$ follows a Gamma distribution. The Gamma shape and rate parameters are 
...
PBrule <- (X + z[1])/(E + z[2])
NPBrule <- f$dy
plot(PBrule, NPBrule, cex = 0.5, xlab = "P-EBayes", ylab = "NP-EBayes")
abline(c(0,1))
...
for(g in 1:2){
   ...
   legend("topleft",c("Male","Female"),lty = rep(1,2), lwd = 1.5, col=cols)
   }
...
lines(day, hW, col = 2)
legend("topright", legend = c("NPMLE", "Gamma"), col = 2:1, lty = 1:2)
