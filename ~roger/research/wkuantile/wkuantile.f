*  This is a slightly modified version avoiding passing logicals from R
      SUBROUTINE wkuantile( FRACT, EL, X, W, IP, Q)
*+
*  Name:
*     wkuantile
...
*  Invocation:
*     CALL wkuantile( USEWT, INTERP, FRACT, EL, X, W, IP, Q)
...
*     29-JULY-2003 (RK):
*        adapted for use in R
*     30-DEC-2020 (RK): 
*         made USEWT and INTERP TRUE 
...
*  Arguments Given:
      LOGICAL USEWT
      PARAMETER (USEWT = .TRUE.)
      LOGICAL INTERP
      PARAMETER (INTERP = .TRUE.)
...
*     PARAMETER ( MXSTK = 100 )
      PARAMETER ( MXSTK = 100 )
      DOUBLE PRECISION HALF                  ! One half, in the required precision
      PARAMETER ( HALF = 1.0D0 / 2.0D0 )
 
*  Local variables:
      DOUBLE PRECISION ALPHA              ! Interpolation fraction
      DOUBLE PRECISION EXTRA              ! Extra cumulative weight required
      DOUBLE PRECISION T                  ! Target value when partitioning
      DOUBLE PRECISION WL                 ! Sum of `left' weights for iteration
      DOUBLE PRECISION WLEFT              ! Sum of `left' sorted weights
      DOUBLE PRECISION WMID               ! Sum of weights of unsorted data
      DOUBLE PRECISION WR                 ! Sum of `right' weights for iteration
      DOUBLE PRECISION WRIGHT             ! Sum of `right' sorted weights
      DOUBLE PRECISION WTARG              ! Target weight for quantile
      ...
      INTEGER ZD                 ! Used for estimating sample size
      INTEGER STK                ! Stack pointer
      INTEGER G                  ! Used for estimating sample size
      DOUBLE PRECISION Z         ! Used for estimating sample size
...
*  sum of weights.  Initialise the target rank to this value.
      KZERO = IDNINT( EL * MIN( MAX ( 0.0D0, FRACT ), 1.0D0 ) )
      K = KZERO
...
*  paper should be consulted for details.)
         Z = DLOG( DBLE( N ) )
         S = IDINT( HALF * DEXP( ( 2.0D0 / 3.0D0 ) * Z ) )
 
*  This is a modification recommended by the algorithm assessor:
 
*  Original:
         ZD = INT( HALF * SQRT( Z * DBLE( S * ( N - S ) / N ) ) *
     :       SIGN( 1, I - N / 2) )
...
*  Modified to:
         ...
         CALL INTPR("G",1,G,1)
         CALL DBLEPR("Z",1,Z,1)
         SD = INT( 0.1D0 * DSQRT( Z * REAL( S * ( N - S ) / N ) ) *
     :           ( 2 * I / N - 1 ) )
         CALL INTPR("SD",2,SD,1)
...
*  elements.
            I1 = MAX( 1, L - 1 )
            I2 = MIN( R + 1, EL )
            ALPHA = ( EL * MIN( MAX( 0.0D0, FRACT ), 1.0D0 ) ) -
     :              ( I1 - HALF )
            ALPHA = MIN( MAX( 0.0D0, ALPHA ), 1.0D0 )
 
*  To obtain the result, either interpolate between the adjacent
*  values...
            IF ( INTERP ) THEN
               Q = ( 1.0D0 - ALPHA ) * X( IP( I1 ) ) +
     :             ALPHA * X( IP( I2 ) )
...
*  for all elements smaller than the final result should have.
         WTARG = MIN( MAX( 0.0D0, FRACT ), 1.0D0 ) *
     :           ( WLEFT + WL + WR + WRIGHT )
...
*  element if there are no weights left in this region.
         IF ( WMID .NE. 0.0D0 ) THEN
            K = IDNINT( ( R - L + 2 ) * ( EXTRA / WMID ) ) + L - 1
         ELSE
            K = ( L + R ) / 2
         END IF
