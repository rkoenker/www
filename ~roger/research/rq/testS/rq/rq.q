...
"bandwidth.rq" <- 
function(p, n, hs = T, alpha = 0.90000000000000002)
{
...
"latex.table" <- 
function(x, file = as.character(substitute(x)), rowlabel = file, rowlabel.just
	 = "l", cgroup, n.cgroup, rgroup, n.rgroup = NULL, digits, dec, rdec,
	cdec, append = F, dcolumn = F, cdot = F, longtable = F, table.env = T,
...
function(object, caption = "caption goes here.", digits = 3, file = "a")
{
...
"rq.fit.fn" <- 
function(x, y, tau = 0.5, int = F, beta = 0.99995000000000001, eps = 
...
#
function(x, y, tau = 0.5, int = F, Mm.factor = 0.80000000000000004, 
	max.bad.fixup = 3, eps = 9.9999999999999995e-07)
{
...
#
function(formula, taus = c(0.050000000000000003, 0.25, 0.5, 0.75, 
	0.94999999999999996), method = "br", ...)
{
	m <- length(taus)
	tab <- NULL
	for(i in 1:m) {
